...
import           Miso.Event (on, Decoder(..), DecodeTarget(..), Events)
----------------------------------------------------------------------------
viewEvents :: Events
viewEvents
  = M.fromList
...
--
-- view :: Model -> View Action
-- view model = view_ [ onTap HandleTouch ]
--
-- update :: Action -> Effect Model Action
-- update (HandleTouch TouchEvent {..}) = do
--   io_ (consoleLog "touch event received")
...
-- @
--
onTap :: (TouchEvent -> action) -> Attribute action
onTap action = on "tap" touchDecoder (\x _ -> action x)
----------------------------------------------------------------------------
